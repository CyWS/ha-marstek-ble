"""Tests for Jupiter AC-output energy counter semantics."""
...
def test_jupiter_output_energy_entities_keep_stable_keys_and_use_output_names() -> None:
    data = JupiterData()
    ...
    expected = {
        "daily_discharge_energy": (
            "Daily AC Output Energy",
            ("energy", "daily_ac_output_energy"),
        ),
        "monthly_discharge_energy": (
            "Monthly AC Output Energy",
            ("energy", "monthly_ac_output_energy"),
        ),
        "local_total_discharge_energy": (
            "Local Total AC Output Energy",
            ("energy", "local_total_ac_output_energy"),
        ),
    }
    ...
    assert "daily_ac_output_energy" not in sensors
    assert "monthly_ac_output_energy" not in sensors
    assert "local_total_ac_output_energy" not in sensors
...
def test_jupiter_output_energy_counters_parse_from_runtime_and_detail_packets() -> None:
    ...
    assert data.energy.daily_ac_output_energy == pytest.approx(12.34)
    assert data.energy.monthly_ac_output_energy == pytest.approx(56.78)
    ...
    assert data.energy.daily_ac_output_energy == pytest.approx(23.45)
    assert data.energy.local_total_ac_output_energy == pytest.approx(67.89)
    assert data.energy.monthly_ac_output_energy == pytest.approx(34.56)


def test_jupiter_legacy_discharge_aliases_follow_renamed_output_energy_fields() -> None:
